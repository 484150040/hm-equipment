...
import com.hm.digital.equipment.controller.dahua.dto.LoginDto;
import com.hm.digital.equipment.controller.dahua.enums.MethodEnum;
...
public class HangarController {
  ...
  @PostConstruct
  public void init() {
    host = configsServices.getValue(getCofig(ConfigEnum.HANGAR.getKey())).get(0).getValue();
    //登录
    String username = configsServices.getValue(getCofig(ConfigEnum.HANGAR_USERNAME.getKey())).get(0).getValue();
    String password = configsServices.getValue(getCofig(ConfigEnum.HANGAR_PASSWORD.getKey())).get(0).getValue();
    ...
    droneVo.setMethod(MethodEnum.FN_GETNESTINFO.getValue());
    ...
    droneVo.setMethod(MethodEnum.FN_GETMQTTCONFIG.getValue());
    ...
    if (!CollectionUtils.isEmpty(droneMqttmap)) {
      if (Integer.valueOf(droneMqttmap.get("code").toString()) == 200) {
        ...
        droneVo.setMethod(MethodEnum.FN_SETMQTTCONFIG.getValue());
